...
.PHONY: psql-test-conn
psql-test-conn: ## Run psql to test if it can connect to go-pg-router
	psql -h localhost -p 3000 -U anyuser -d "dbname=anydb"

.PHONY: psql-test-conn-no-ssh
psql-test-conn-no-ssh: ## Run psql with sslmode disable to test if it can connect to go-pg-router
